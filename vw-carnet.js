...
function CarNetLogon(callback) { //retrieve Token for the respective user
    const myUrl = `https://${requestHostAuth}/v1/token`;
    const myFormdata = {
        'grant_type': 'password',
    ...
    request.post({ url: myUrl, form: myFormdata, headers: requestHeaders, json: true }, function (error, response, result) {
      //adapter.log.info('LogOn_Result: ' +  JSON.stringify(result));
        if (isRequestOk('CarNetLogin', error, response, result)) {
            myAuthHeaders.Authorization = 'Bearer ' + result.access_token;
            //adapter.log.info('LogOn_Result: ' +  myAuthHeaders.Authorization);
            return callback(true);
        } else {
...
function RetrieveVehicles(callback) { //retrieve VIN of the first vehicle (Fahrgestellnummer)
    const myVehicleID = 0;
    const myUrl = `https://${requestHost}/myaudi/vehicle-management/v1/vehicles`;
    ...
    if (adapter.config.VIN === '') {
        request.get({ url: myUrl, headers: myAuthHeaders, json: true }, function (error, response, result) {
            ...
            myVIN = result.vehicles[0].vin;
            return callback('Count: ' + result.vehicles.vin.length);
        });
    } else {
...
function RetrieveVehicleData_VINValid(callback) {
    let myVINIsValid = true; //eigentlich false, nur zu testzwecken true
    //const myUrl = `https://${requestHost}/myaudi/vehicle-management/v1/vehicles/${myVIN}`;
    //request.get({ url: myUrl, headers: myAuthHeaders, json: true }, function (error, response, result) {
    //    adapter.log.info('Retrieve brand and country: ' + JSON.stringify(result));
    //    try {
    //        if (result.vehicle[0].vin === myVIN) {
    //            myVINIsValid = true;
                //VWCarNet_Brand = result.vehicleData.brand;
                //VWCarNet_Country = result.vehicleData.country; Not existing anymore
    //        }
    //    }
    //    catch (ex) {
    //        myVINIsValid = false;
